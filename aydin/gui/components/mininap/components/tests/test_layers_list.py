...
def test_move_selected():
    ...
    assert list(layers) == [layer_a, layer_b, layer_c, layer_d, layer_e, layer_f]
    ...
    # Move selection together to middle
    layers.move_selected(1, 2)
    assert list(layers) == [layer_a, layer_c, layer_b, layer_e, layer_d, layer_f]
    assert [l.selected for l in layers] == [False, False, True, True, False, False]
