...
class Masker:
    ...
    def __init__(
        ...
        if density:
            ...
            self.density = 1.0 / self.n_masks

            self.inference_fixed_mask = (
                torch.cuda.FloatTensor((1, 1)).uniform_(0, 1) < self.density
            ).float()

    def mask(self, X, i, inference=False):

        if self.random:

            if inference:
                if self.inference_fixed_mask.shape != X.shape:
                    self.inference_fixed_mask = (
                        torch.cuda.FloatTensor(X.shape).uniform_(0, 1) < self.density
                    ).float()
                mask = self.inference_fixed_mask
            else:
                mask = (
                    torch.cuda.FloatTensor(X[0, 0].shape).uniform_(0, 1) < self.density
                ).float()
        ...
        if self.mode == 'interpolate':
            ...
            random_tensor = torch.cuda.FloatTensor(X.shape).uniform_(0, 1).to(X.device)
            masked = X * mask_inv + random_tensor * mask
    ...
    def infer_full_image(self, X, model):
        ...
        if self.infer_single_pass:
            ...
            acc_tensor = torch.zeros(net_output.shape).to(X.device)

            for i in range(self.n_masks):
                net_input, mask = self.mask(X, i, inference=True)
                net_output = model(net_input)
                acc_tensor = acc_tensor + net_output * mask
