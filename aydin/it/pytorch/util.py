...
def plot_grid(images, height, width, cmap='Greys_r'):
    ...
    for i in range(height):
        for j in range(width):
            if height > 1:
                ax[i, j].imshow(image_grid[i, j], cmap=cmap)
            else:
                ax[j].imshow(image_grid[i, j], cmap=cmap)
    fig


def plot_images(image_list):
    images = np.concatenate([im[np.newaxis] for im in image_list])
    plot_grid(images, 1, len(image_list))
...
def getbestgpu(forceid=None):
    ...
    if not forceid == None:
        idbest = forceid
        print("--> GPU device was overridden (forceid=%d)" % idbest)
