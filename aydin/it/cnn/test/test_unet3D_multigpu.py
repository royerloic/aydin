...
ngpus = len(K.tensorflow_backend._get_available_gpus())
...
class ImageTranslatorCNN:
    ...
    def train(
        ...
        self.parallel_model = multi_gpu_model(self.model, gpus=num_gpu)
        self.parallel_model.compile(loss='mse', optimizer='Adam')

        if self.supervised:
            history = self.parallel_model.fit(
            ...
            history = self.parallel_model.fit_generator(
                maskedgen(
                    self.input_dim[:-1],
                    mask_shape,
                    normalised_input_image,
    ...
    def translate(self, input_image, batch_dim=None):
        ...
        if not self.shiftconv and not self.supervised:
            translated_image = self.parallel_model.predict(
...
print(image.shape)
print(
    f'Training data coverage: {numpy.prod(image.shape) / numpy.prod(image_size[1:]) * 100:.2f}%'
)
image = image.astype(numpy.float32)

# image = numpy.expand_dims(numpy.expand_dims(image, axis=-1), axis=0)
image = numpy.expand_dims(image, axis=-1)

# batch size for multi_gpu training, meaning batch size for each gpu is 1 / # of gpus.
batch_size = 15
...
it = ImageTranslatorCNN(
    ...
    supervised=False,
    shiftconv=True,
    rot_batch=batch_size // ngpus,
)

start = time.time()
denoised = it.train(image, image, batch_size=batch_size, num_gpu=ngpus)
