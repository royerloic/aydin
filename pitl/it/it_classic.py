...
class ImageTranslatorClassic(ImageTranslatorBase):
    ...
    def __init__(
        ...
        self.receptive_field_radius = (
            self.feature_generator.get_receptive_field_radius()
        )
    ...
    def train(
        ...
        dataset_size_in_bytes = (
            input_image.size * input_image.itemsize + 0
            if self.self_supervised
            else (target_image.size * target_image.itemsize)
        )
        dataset_size_for_features_in_bytes = self.feature_generator.get_needed_mem(
            input_image.size
        )
        ...
        if self.debug:
            ...
            print(
                f'Dataset is {(dataset_size_for_features_in_bytes/1E6)} MB when computing features (float type).'
            )
        ...
        if self.debug:
            ...
            print(
                f'There is {int(free_feature_mem_in_bytes/1E6)} MB of free feature gen memory (GPU)'
            )
        ...
        is_enough_memory = (
            ...
            and dataset_size_for_features_in_bytes <= effective_batch_size
        )
        ...
        if (batch_size is None) and is_enough_memory:
            ...
            inferred_image = self._train(
                input_image, target_image, batch_dims, train_test_ratio, batch=False
            )
            return inferred_image
            ...
            self.batch_strategy = self._get_batching_strategy(
                batch_dims, dataset_size_in_bytes, effective_batch_size, shape
            )
    ...
    def _get_batching_strategy(
        self, batch_dims, dataset_size_in_bytes, effective_batch_size, shape
    ):
    ...
    def _train(
        self, input_image, target_image, batch_dims, train_test_ratio, batch=False
    ):
    ...
    def translate(self, input_image, batch_dims=None):
        ...
        if self.batch_strategy is None:
            features = self._compute_features(
                input_image, self.self_supervised, batch_dims
            )
            ...
            for slice_margin_tuple, slice_tuple in zip(
                ...
                features = self._compute_features(
                    input_image_batch, self.self_supervised, batch_dims
                )

                inferred_image_batch = self._predict_from_features(
                    features, input_image_shape=input_image_batch.shape
                )
                ...
                inferred_image[slice_tuple] = inferred_image_batch[
    ...
    def get_margins(self, shape, batch_strategy):
        ...
        margins = tuple(
            min(self.receptive_field_radius, (dim // split) // 3)
