...
def get_weightnorm_params_and_grads(p, g):
    ...
    V_norm = K.sqrt(K.reduce_sum(K.square(V), norm_axes))
    g_param = V_scaler * V_norm

    # get grad in V,g parameters
    grad_g = K.reduce_sum(g * V, norm_axes) / V_norm
...
def add_weightnorm_param_updates(updates, new_V_param, new_g_param, W, V_scaler):
    ...
    new_V_norm = K.sqrt(K.reduce_sum(K.square(new_V_param), norm_axes))
...
def data_based_init(model, input):
    ...
    for l, o, W, b in layer_output_weight_bias:
        ...
        m, v = K.nn.moments(o, [i for i in range(len(o.get_shape()) - 1)])
        s = K.sqrt(v + 1e-10)
        updates = K.group(
            W.assign(W / K.reshape(s, [1] * (len(W.get_shape()) - 1) + [-1])),
            b.assign((b - m) / s),
        )
